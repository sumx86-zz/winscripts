...
#define GET_THREAD_ROUTINE()\
    (LPVOID) GetProcAddress( GetModuleHandle(TEXT("kernel32")), "LoadLibrary")
...
bool dllinject::InjectDll( const std::wstring& dllpath ) {
    ...
    LPTHREAD_START_ROUTINE routine = reinterpret_cast<LPTHREAD_START_ROUTINE>(GET_THREAD_ROUTINE());
